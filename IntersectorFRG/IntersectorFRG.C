...
void ClosestTriangle::start(Vec3D xp)
{
  x = xp;
  isFirst = true;
  bestTrId = -1;
...
void ClosestTriangle::checkTriangle(int trId)
{
  double dist;
...
void ClosestTriangle::checkVertex(int ip1, int trId, double trDist)
{
  ...
  if (n1 == ip1 && n2 < 0)
  {
    ...
    if ((thisSign != isPositive || !isConsistent) /* && !hasBeenFixed*/)
    { // need to figure out the correct sign...
      isConsistent = false;
      if (repeated1 != -1)
      { // this triangle and another traversed triangle share an edge that has this damn node.
...
bool ClosestTriangle::checkEdge(int trId, int ip1, int ip2, int p3, double trDist)
{
  int p1, p2;
  if (ip1 < ip2)
  {
  ...
  if (n1 == p1 && n2 == p2)
  { //<! If we've already looked at this edge before
...
double ClosestTriangle::findSignedVertexDistance()
{
  ...
  if (rr < 1.0e-14)
  {
    fprintf(stderr, "ERROR: (in IntersectorFRG) distance = %e.\n", rr);
    exit(-1);
  }
                                       ...
                                       int nElems, int (*abc)[3]) : DistLevelSetStructure(), iod(iodata)
{
  ...
  if (externalSI)
    comm->fprintf(stdout, " +++ Using external-based surrogate interface +++ \n");
...
void DistIntersectorFRG::init(char *solidSurface, char *restartSolidSurface, double XScale)
{
  ...
  if (topFile == NULL)
  {
    com->fprintf(stderr, "Embedded structure surface mesh doesn't exist :(\n");
    exit(1);
  }
  ...
  if (restartSolidSurface[0] != 0)
  {
    ...
    if (resTopFile == NULL)
    {
      com->fprintf(stderr, "restart topFile doesn't exist.\n");
      exit(1);
    }
...
void DistIntersectorFRG::updatebc()
{
  ...
  if (rotOwn)
  {
    ...
    {
      if (rotOwn[k] >= 0)
      { // node belongs to a (potential) "rotating" surface
        map<int, RotationData *>::iterator it = rotationMap.find(rotOwn[k]);
        if (it != rotationMap.end())
        { // the rotation data have been defined
        ...
        { // no rotation data -> use velocity from mesh motion if any
          Xsdot[k][0] = Xsdot[k][0];
          Xsdot[k][1] = Xsdot[k][1];
          Xsdot[k][2] = Xsdot[k][2];
        }
...
void DistIntersectorFRG::initializePhysBAM()
...
bool DistIntersectorFRG::checkTriangulatedSurface()
{
  ...
  for (int iTriangle = 0; iTriangle < numStElems; iTriangle++)
  {
    ...
    from1 = stElem[iTriangle][0];
    to1 = stElem[iTriangle][1];
    found1 = false;
    from2 = stElem[iTriangle][1];
    to2 = stElem[iTriangle][2];
    found2 = false;
    from3 = stElem[iTriangle][2];
    to3 = stElem[iTriangle][0];
    found3 = false;
...
void DistIntersectorFRG::buildSolidNormals()
{
  ...
  for (int iTriangle = 0; iTriangle < numStElems; iTriangle++)
  {
    ...
    if (interpolatedNormal)
    { // compute nodal normal (weighted by area)
...
void DistIntersectorFRG::initialize(Domain *d, DistSVec<double, 3> &X, DistSVec<double, 3> &Xn, IoData &iod, DistVec<int> *point_based_id, DistVec<int> *oldStatus)
{
  if (this->numFluid < 1)
  {
...
void DistIntersectorFRG::findPoly()
{
  if (!poly)
  {
...
#pragma omp parallel for
  ...
  if (!twoPhase)
  {
    ...
    if (subdXing)
    {                   //this happens if the structure is entering/leaving a subdomain. Rarely happens but needs to be delt with...
...
#pragma omp parallel for
  ...
  while (1)
  { //get out only when all nodes are decided
  ...
  for (int l = 0; l < edges.size(); l++)
  {
    if (edge_intersects[l])
    {
      ...
      if ((fabs(X[i][0] - 0.2127) < 1.0e-3 && fabs(X[i][1] + 0.01017) < 1.0e-4 /* && fabs(X[i][2] - 0.05) < 1.0e-4*/) ||
          (fabs(X[j][0] - 0.2127) < 1.0e-3 && fabs(X[j][1] + 0.01017) < 1.0e-4 /* && fabs(X[j][2] - 0.05) < 1.0e-4*/))
        isOK1 = true;
      bool isOK2 = false;
      if ((fabs(X[i][0] - 0.2127) < 1.0e-3 && fabs(X[i][1] + 0.02034) < 1.0e-4 /* && fabs(X[i][2] - 0.05) < 1.0e-4*/) ||
          (fabs(X[j][0] - 0.2127) < 1.0e-3 && fabs(X[j][1] + 0.02034) < 1.0e-4 /* && fabs(X[j][2] - 0.05) < 1.0e-4*/))
        isOK2 = true;
      ...
      if (alpha_1 > 0.5 && is_active[i])
        is_active[i] = false;
      if (alpha_2 > 0.5 && is_active[j])
        is_active[j] = false;
    }
  }
}
...
#pragma omp parallel for
  ...
  if (total)
  { //still have undecided nodes. They must be ghost nodes (i.e. covered by solid).
...
#pragma omp parallel for
  for (int iSub = 0; iSub < numLocSub; iSub++)
    ...
    {
      if ((*status)(iSub)[i] == IntersectorFRG::OUTSIDE)
      { //this means its real status hasn't been decided.
...
void IntersectorFRG::floodFill(SubDomain &sub, int &nUndecided)
{
  ...
  { //still have seeds not used
    ...
    for (int i = 0; i < nToN.num(cur); ++i)
    {
      ...
      else if (status[nToN[cur][i]] != curStatus && (curLevel != 0 || level[nToN[cur][i]] != 0))
...
IntersectorFRG::IntersectorFRG(SubDomain &sub, SVec<double, 3> &X,
                               Vec<int> &stat0, DistIntersectorFRG &distInt) : LevelSetStructure((*distInt.status)(sub.getLocSubNum()), (*distInt.distance)(sub.getLocSubNum()),
...
int IntersectorFRG::findIntersections(SVec<double, 3> &X, bool useScope)
{
  ...
  for (int l = 0; l < edges.size(); l++)
  {
    ...
    if (res1.triangleID < 0 && useScope)
    { // try global intersector as a 'fail-safe'.
...
IntersectorFRG::getLevelSetDataAtEdgeCenter(double t, int l, bool i_less_j, double *Xr, double *Xg)
{
  ...
  { //we really have no intersection for this edge!
    int(*ptr)[2] = edges.getPtr();
    int i = i_less_j ? ptr[l][0] : ptr[l][1],
        j = i_less_j ? ptr[l][1] : ptr[l][0];
    fprintf(stderr,
      "ERROR: intersection between node %d and node %d can not be detected.\n",
...
void DistIntersectorFRG::updateXb(double epsilon)
{
