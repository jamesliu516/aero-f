					 ...
					 double (*dSdU)[neq][neq])
{
  ...
  double chi = mutilde/mul;
  double dchi = dmutilde/mul; // double dchi = 1.0/mul;
  double chi2 = chi*chi;
  double chi3 = chi*chi*chi;
  double mu5neg, drdxneg, drdyneg, drdzneg, fn, dfn;
  if (negSA) {
    fn = (cn1+chi3)/(cn1-chi3);
    dfn = 6.0*chi2*cn1/((cn1-chi3)*(cn1-chi3))*dchi;
    drdxneg = oosigma * 0.25 * (fn + dfn * mutilde) * dnutildedx;  // dmutilde = 1.0
    drdyneg = oosigma * 0.25 * (fn + dfn * mutilde) * dnutildedy;
    drdzneg = oosigma * 0.25 * (fn + dfn * mutilde) * dnutildedz;
    mu5neg = oosigma * (mul + fn*mutilde);
  }
  ...
  for (k=0; k<4; ++k) {
    if (negSA && V[k][5] < 0.0) {
      ...
      dRdU[k][2][shift][shift] = drdzneg + nu * dp1dxj[k][2];
    }
    else {
      nu = mu5 / V[k][0];
  ...
  double P = cb1 * Stilde * dmutilde;
  double dP = cb1 * dStilde * mutilde;
  double D = cw1 * oorho * ood2wall2 * fw * mutilde * dmutilde;
  double dD = cw1 * oorho * ood2wall2 * (fw * dmutilde * mutilde +  dfw * mutilde * mutilde);
  // // double s00 = 0.25 * (max(D - P, 0.0) + max(dD - dP, 0.0)); // why the max?
  double s00 = 0.25 * (D + dD - (P + dP));
  ...
  double s00neg, coef5neg;
  if (negSA) {
    double Pneg = cb1 * s * dmutilde;
    double dPneg = 0.0;
    double Dneg = - cw1 * oorho * ood2wall2 * mutilde * dmutilde;
    double dDneg = - cw1 * oorho * ood2wall2 * dmutilde * mutilde;
    s00neg = 0.25 * (Dneg + dDneg - (Pneg + dPneg));

    coef5neg = 0.25 * oosigma * (fn + dfn*mutilde) *
      (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz);
  }
					 ...
					 double (*dSdU)[neq][neq])
{
  ...
  double mu5neg, dmu5neg[4][6];
  if (negSA) {
    double dfn[4][6], fn = (cn1+chi3)/(cn1-chi3);
    double coef1 = 6.0*chi*chi*cn1/((cn1-chi3)*(cn1-chi3));
    for (k=0; k<4; ++k) {
      dfn[k][0] = coef1*dchi[k][0];
      dfn[k][1] = coef1*dchi[k][1];
      dfn[k][2] = coef1*dchi[k][2];
      dfn[k][3] = coef1*dchi[k][3];
      dfn[k][4] = coef1*dchi[k][4];
      dfn[k][5] = coef1*dchi[k][5];

      dmu5neg[k][0] = oosigma * (dmul[k][0] + fn*dmutilde[k][0] + dfn[k][0]*mutilde);
      dmu5neg[k][1] = oosigma * (dmul[k][1] + fn*dmutilde[k][1] + dfn[k][1]*mutilde);
      dmu5neg[k][2] = oosigma * (dmul[k][2] + fn*dmutilde[k][2] + dfn[k][2]*mutilde);
      dmu5neg[k][3] = oosigma * (dmul[k][3] + fn*dmutilde[k][3] + dfn[k][3]*mutilde);
      dmu5neg[k][4] = oosigma * (dmul[k][4] + fn*dmutilde[k][4] + dfn[k][4]*mutilde);
      dmu5neg[k][5] = oosigma * (dmul[k][5] + fn*dmutilde[k][5] + dfn[k][5]*mutilde);
    }
  }
  ...
  for (k=0; k<4; ++k) {
    if (negSA && V[k][5] < 0.0) {
      ...
      dRdU[k][0][5][3] = dmu5neg[k][3] * dnutildedx;
      dRdU[k][0][5][4] = dmu5neg[k][4] * dnutildedx;
      dRdU[k][0][5][5] = dmu5neg[k][5] * dnutildedx + mu5neg * dp1dxj[k][0] / V[k][0];

      dRdU[k][1][5][0] = dmu5neg[k][0] * dnutildedy - mu5neg * dp1dxj[k][1] * V[k][5] / V[k][0];
      dRdU[k][1][5][1] = dmu5neg[k][1] * dnutildedy;
      dRdU[k][1][5][2] = dmu5neg[k][2] * dnutildedy;
      dRdU[k][1][5][3] = dmu5neg[k][3] * dnutildedy;
      dRdU[k][1][5][4] = dmu5neg[k][4] * dnutildedy;
      dRdU[k][1][5][5] = dmu5neg[k][5] * dnutildedy + mu5neg * dp1dxj[k][1] / V[k][0];

      dRdU[k][2][5][0] = dmu5neg[k][0] * dnutildedz - mu5neg * dp1dxj[k][2] * V[k][5] / V[k][0];
      dRdU[k][2][5][1] = dmu5neg[k][1] * dnutildedz;
      dRdU[k][2][5][2] = dmu5neg[k][2] * dnutildedz;
      dRdU[k][2][5][3] = dmu5neg[k][3] * dnutildedz;
      dRdU[k][2][5][4] = dmu5neg[k][4] * dnutildedz;
      dRdU[k][2][5][5] = dmu5neg[k][5] * dnutildedz + mu5neg * dp1dxj[k][2] / V[k][0];
    }
    else {
...
//  double DD;          // sjg, 06/2017: missing term from conservation form
  ...
  double dBBneg[4][6], dCCneg[4][6], dDDneg[4][6];
  if (negSA) {
    for (k=0; k<4; ++k) {
      // BB = cb1 * s * absmutilde;
      dBBneg[k][0] = cb1 * ds[k][0] * mutilde + cb1 * s * dmutilde[k][0];
      dBBneg[k][1] = cb1 * ds[k][1] * mutilde + cb1 * s * dmutilde[k][1];
      dBBneg[k][2] = cb1 * ds[k][2] * mutilde + cb1 * s * dmutilde[k][2];
      dBBneg[k][3] = cb1 * ds[k][3] * mutilde + cb1 * s * dmutilde[k][3];
      dBBneg[k][4] = cb1 * ds[k][4] * mutilde + cb1 * s * dmutilde[k][4];
      dBBneg[k][5] = cb1 * ds[k][5] * mutilde + cb1 * s * dmutilde[k][5];

      // CC = - cw1 * oorho * maxmutilde*maxmutilde * ood2wall2;
      dCCneg[k][0] = cw1 * doorho[k][0] * mutilde*mutilde * ood2wall2 + cw1 * oorho * 2.0*mutilde*dmutilde[k][0] * ood2wall2;
      dCCneg[k][1] = cw1 * doorho[k][1] * mutilde*mutilde * ood2wall2 + cw1 * oorho * 2.0*mutilde*dmutilde[k][1] * ood2wall2;
      dCCneg[k][2] = cw1 * doorho[k][2] * mutilde*mutilde * ood2wall2 + cw1 * oorho * 2.0*mutilde*dmutilde[k][2] * ood2wall2;
      dCCneg[k][3] = cw1 * doorho[k][3] * mutilde*mutilde * ood2wall2 + cw1 * oorho * 2.0*mutilde*dmutilde[k][3] * ood2wall2;
      dCCneg[k][4] = cw1 * doorho[k][4] * mutilde*mutilde * ood2wall2 + cw1 * oorho * 2.0*mutilde*dmutilde[k][4] * ood2wall2;
      dCCneg[k][5] = cw1 * doorho[k][5] * mutilde*mutilde * ood2wall2 + cw1 * oorho * 2.0*mutilde*dmutilde[k][5] * ood2wall2;

      // DD = - oorho * mu5 * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz);
      dDDneg[k][0] = - (doorho[k][0] * mu5neg * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * dmu5neg[k][0] * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * mu5neg * (dnutildedx*dp1dxj[k][0] + dnutildedy*dp1dxj[k][1] + dnutildedz*dp1dxj[k][2]))
        + oorho * mu5neg * (drhodx*dp1dxj[k][0]*V[k][5] + drhody*dp1dxj[k][1]*V[k][5] + drhodz*dp1dxj[k][2]*V[k][5]) / V[k][0];
      dDDneg[k][1] = - (doorho[k][1] * mu5neg * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * dmu5neg[k][1] * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz));
      dDDneg[k][2] = - (doorho[k][2] * mu5neg * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * dmu5neg[k][2] * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz));
      dDDneg[k][3] = - (doorho[k][3] * mu5neg * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * dmu5neg[k][3] * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz));
      dDDneg[k][4] = - (doorho[k][4] * mu5neg * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * dmu5neg[k][4] * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz));
      dDDneg[k][5] = - (doorho[k][5] * mu5neg * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * dmu5neg[k][5] * (dnutildedx*drhodx + dnutildedy*drhody + dnutildedz*drhodz)
        + oorho * mu5neg * (drhodx*dp1dxj[k][0] + drhody*dp1dxj[k][1] + drhodz*dp1dxj[k][2]) / V[k][0]);
    }
  }
  ...
  for (k=0; k<4; ++k) {
    if (negSA && V[k][5] < 0.0) {
      ...
      dSdU[k][5][3] = dAA[k][3] + dBBneg[k][3] + dCCneg[k][3] + dDDneg[k][3];
      dSdU[k][5][4] = dAA[k][4] + dBBneg[k][4] + dCCneg[k][4] + dDDneg[k][4];
      dSdU[k][5][5] = dAA[k][5] + dBBneg[k][5] + dCCneg[k][5] + dDDneg[k][5];
    }
    else {
