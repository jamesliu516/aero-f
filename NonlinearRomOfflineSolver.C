...
//------------------------------------------------------------------------------
void startNonlinearRomOfflineSolver(Communicator *com, IoData &ioData, Domain &domain)
{

   if (ioData.eqs.type == EquationsData::EULER ) {
    NonlinearRomOfflineSolver<5> offlineSolver(com, ioData, domain);
    ...
    if (ioData.eqs.tc.type == TurbulenceClosureData::NONE ||
      ...
      if (ioData.eqs.tc.tm.type == TurbulenceModelData::ONE_EQUATION_SPALART_ALLMARAS ||
        ...
        NonlinearRomOfflineSolver<6> offlineSolver(com, ioData, domain);
