...
void Domain::computeFiniteVolumeTermLS(FluxFcn** fluxFcn, RecFcn* recFcn, RecFcn* recFcnLS,
                                     ...
                                     DistSVec<double,1>& Phi, DistVec<double> &PhiF)
{
  double t0 = timer->getTime();
  int iSub;
...
template<int dim>
void Domain::recomputeRHS(VarFcn* vf, DistSVec<double,dim> &V, DistVec<double> &Phi,
...
#pragma omp parallel for
  for ( iSub = 0; iSub < numLocSub; iSub++){
    Extrapolation<dim>* lxpol = (xpol) ? &((*xpol)(iSub)) : 0;
    subDomain[iSub]->recomputeRHS(vf, V(iSub), Phi(iSub), rhs(iSub), lxpol,
                                  bcData(iSub), geoState(iSub), X(iSub));
    subDomain[iSub]->sndInletRhsData(*inletRhsPat, rhs.subData(iSub));
  }
...
template<int dim>
void Domain::computeForceLoad(DistSVec<double,3> &X, double (*Fs)[3], int sizeFs, 
                              DistLevelSetStructure *distLSS,
                              DistSVec<double,dim> &Wstarij, DistSVec<double,dim> &Wstarji)
{
...
#pragma omp parallel for
  for (int iSub=0; iSub<numLocSub; iSub++) {
    ...
    subDomain[iSub]->computeForceLoad(X(iSub), subFs[iSub], sizeFs, (*distLSS)(iSub), 
                                      pstarij(iSub), pstarji(iSub));
  }
