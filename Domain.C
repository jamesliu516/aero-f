...
#pragma omp parallel for
  for (iSub = 0; iSub < numLocSub; ++iSub) {
    subDomain[iSub]->computeTimeStep(fet, varFcn, geoState(iSub), X(iSub), V(iSub), dt(iSub), idti(iSub), idtv(iSub), dtau(iSub), tprec);
...
#pragma omp parallel for
  for (iSub = 0; iSub < numLocSub; ++iSub) {
	  ...
	  subDomain[iSub]->computeWeightsLeastSquaresNodePartForEmbeddedStruct(
			  (*count)(iSub),(*R)(iSub));
  }
...
template<int dim>
void Domain::populateGhostPoints(DistVec<GhostPoint<dim>*> *ghostPoints, DistSVec<double,3> &X, DistSVec<double,dim> &U, DistNodalGrad<dim, double> *ngrad, VarFcn *varFcn,DistLevelSetStructure *distLSS,DistVec<int> &tag)
{
  int iSub;
#pragma omp parallel for
  for (iSub = 0; iSub < numLocSub; ++iSub) 
    {
      subDomain[iSub]->populateGhostPoints((*ghostPoints)(iSub),X(iSub),U(iSub),(*ngrad)(iSub),varFcn,(*distLSS)(iSub),tag(iSub));
    }
...
#pragma omp parallel for
  for (int iSub=0; iSub<numLocSub; iSub++) {
    ...
    subDomain[iSub]->computeRecSurfBasedForceLoad(forceApp, orderOfAccuracy, X(iSub), subFs[iSub], sizeFs,
						     (*distLSS)(iSub), pInfty, 
						     Wstarij(iSub), Wstarji(iSub), V(iSub), gp, postFcn, vf, fid?&((*fid)(iSub)):0);
  }
  for (int is=0; is<sizeFs; is++) {
    Fs[is][0] = subFs[0][is][0];
    Fs[is][1] = subFs[0][is][1];
