  ...
  for (int iElem=0; iElem<elems.size(); iElem++) {
    ...
    for(int i=0; i < numberOfPolygons; ++i){
      ...
      for(int k=0; k<nEdges; ++k) {
        ...
        if(!applyRealForce)
          Vinter[k][4] = pInfty; //this is enough.
        else {
      ...
      switch(nEdges){
        case 3: //got a triangle
          ...
          postFcn->computeForceEmbedded(order,dp1dxj,Xface,nf,d2w,0/*Vwall*/,Vface,v,&pInfty,
                                        fi0,fi1,fi2,fv,dPdx,0/*"hydro"*/, applyRealForce);
          if(ghostPoints && applyRealForce) {
            fv *= oneThird;
            fi0+=fv;
            fi1+=fv;
        ...
        case 4: //got a quadrangle. cut it into two triangles.
          ...
          if(dist02<dist13){ // connect 0,2.
            ...
            nf = 0.5*(Xinter[1]-Xinter[0])^(Xinter[2]-Xinter[0]);
            if(nf*(Xinter[1]-start_vertex) <= 0) nf *=-1;
            Xface[0] = &Xinter[0][0];  Vface[0] = &Vinter[0][0];
            Xface[1] = &Xinter[1][0];  Vface[1] = &Vinter[1][0];
            Xface[2] = &Xinter[2][0];  Vface[2] = &Vinter[2][0];
            postFcn->computeForceEmbedded(order,dp1dxj,Xface,nf,d2w,0/*Vwall*/,Vface,v,&pInfty,
                                          fi0,fi1,fi2,fv,dPdx,0/*"hydro"*/, applyRealForce);
            if(ghostPoints && applyRealForce) {
              fv *= oneThird;
            ...
            if(nf*(Xinter[1]-start_vertex) <= 0) nf *=-1;
            Xface[0] = &Xinter[0][0];  Vface[0] = &Vinter[0][0];
            Xface[1] = &Xinter[2][0];  Vface[1] = &Vinter[2][0];
            Xface[2] = &Xinter[3][0];  Vface[2] = &Vinter[3][0];
            postFcn->computeForceEmbedded(order,dp1dxj,Xface,nf,d2w,0/*Vwall*/,Vface,v,&pInfty,
                                          fi0,fi1,fi2,fv,dPdx,0/*"hydro"*/, applyRealForce);
            if(ghostPoints && applyRealForce) {
              fv *= oneThird;
            ...
            if(nf*(Xinter[1]-start_vertex) <= 0) nf *=-1;
            Xface[0] = &Xinter[1][0];  Vface[0] = &Vinter[1][0];
            Xface[1] = &Xinter[2][0];  Vface[1] = &Vinter[2][0];
            Xface[2] = &Xinter[3][0];  Vface[2] = &Vinter[3][0];
            postFcn->computeForceEmbedded(order,dp1dxj,Xface,nf,d2w,0/*Vwall*/,Vface,v,&pInfty,
                                          fi0,fi1,fi2,fv,dPdx,0/*"hydro"*/, applyRealForce);
  ...
  for (int l=0; l<edges.size(); l++) {
    ...
    if (iActive) {
      ...
      if(ghostPoints) {// Viscous Simulation
        ...
        gradP[1] = gradY[i][4];
        gradP[2] = gradZ[i][4];
        flocal   = (v[0][4] - pInfty + 0.5*(gradP*vectorIJ))*normal[l];
