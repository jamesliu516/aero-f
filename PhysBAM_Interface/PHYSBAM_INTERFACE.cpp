...
template<class T> void
Ray_Intersection(const ARRAY<TRIANGLE_3D<T> >& triangle_list,const ARRAY<int>& candidates,const T thickness_over_two,RAY<VECTOR<T,3> >& ray,VECTOR<T,3>& weights, LocalLevelSet *cs)
{
    bool intersected=false;
    for(int t=1;t<=candidates.m;++t){
        RAY<VECTOR<T,3> > ray_temp=ray;bool intersected_temp=intersected;
        if(INTERSECTION::Intersects(ray,triangle_list(candidates(t)),thickness_over_two)) {
            ...
            if(cs){VECTOR<T,3> local_weights;
                triangle_list(ray.aggregate_id).Closest_Point(ray.Point(ray.t_max),local_weights);
		if(cs->getPhi(ray.aggregate_id-1,local_weights.x,local_weights.y) <= (T)0){
...
Point_Inside_Triangle(const ARRAY<TRIANGLE_3D<T> >& triangle_list,const VECTOR<T,3>& point,const T thickness_over_two, int& triangleID, VECTOR<T,3>& weights,
		LocalLevelSet* cs, const ARRAY<int>* candidates=0)
{
    if(candidates){
        for(int i=1;i<=candidates->Size();++i) if(triangle_list((*candidates)(i)).Point_Inside_Triangle(point,thickness_over_two)){
            ...
            triangleID=(*candidates)(i);weights=triangle_list((*candidates)(i)).Barycentric_Coordinates(point);
            if(cs && cs->getPhi(triangleID-1,weights.x,weights.y) <= (T)0){triangleID=triangleID_temp;}
	    else return true;}}
    else {
        for(int i=1;i<=triangle_list.Size();++i) if(triangle_list(i).Point_Inside_Triangle(point,thickness_over_two)){
            int triangleID_temp=triangleID;
            triangleID=i;weights=triangle_list(i).Barycentric_Coordinates(point);
            if(cs && cs->getPhi(triangleID-1,weights.x,weights.y) <= (T)0){triangleID=triangleID_temp;}
	    else return true;}}
    return false;
}
...
Intersection_Helper(const ARRAY<TRIANGLE_3D<T> >& triangle_list,const ARRAY<int>& scope,const ARRAY<int>& candidates,
    ...
    if(!start_node_occluded) // The usual case
        Ray_Intersection(triangle_list,candidates,thickness_over_two,intersection_ray,weights, cs);
        ...
        for(int number_of_attempts=0;number_of_attempts<PERMITTED_ATTEMPTS;++number_of_attempts) {
            ...
            if(Point_Inside_Triangle(triangle_list,start,modified_thickness_over_two,triangleID,weights,cs,&candidates)) {
        ...
        if(triangleID <= 0 && Point_Inside_Triangle(triangle_list,start,modified_thickness_over_two,triangleID,weights,cs)) {
    ...
    if(intersection_ray.intersection_location == RAY<TV>::START_POINT) {
        for(int number_of_attempts=0;number_of_attempts<PERMITTED_ATTEMPTS && intersection_ray.intersection_location == RAY<TV>::START_POINT;++number_of_attempts) {
            ...
            Ray_Intersection(triangle_list,candidates,modified_thickness_over_two,intersection_ray,weights, cs);}
    ...
    if(end_node_occluded && intersection_ray.intersection_location != RAY<TV>::INTERIOR_POINT) {
        ...
        for(int number_of_attempts=0;number_of_attempts<PERMITTED_ATTEMPTS;++number_of_attempts) {
            ++retryAttempts;
            if(Point_Inside_Triangle(triangle_list,end,modified_thickness_over_two,triangleID,weights,cs,&candidates)) {
                Assign_Intersection_Information(scope(triangleID),(T)1-modified_thickness_over_two,weights,result);break;}
            modified_thickness_over_two*=(T)2;}
        if(triangleID <= 0 && Point_Inside_Triangle(triangle_list,end,modified_thickness_over_two,triangleID,weights,cs)) {
